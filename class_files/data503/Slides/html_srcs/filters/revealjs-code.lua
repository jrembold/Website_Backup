--[[
revealjs-code - enable reveal.js code presentation features

Adapted from © 2020 Tim Sokollek
Added inline code blocks by Jed Rembold, 2020
...
function Code(inline)
	if FORMAT == 'revealjs' then
		...
		for _, attribute in ipairs(inline.attributes) do
			attribute_string = string.format('%s="%s"', attribute[1],
			attribute[2])
			if is_pre_tag_attribute(attribute[1]) then
				table.insert(pre_tag_attributes, attribute_string)
		...
		local html = string.format('<code %s>%s</code>',
		table.concat(code_tag_attributes, ' '),
		inline.text)
