...
function rlight(r2,c2) {
	k2 = b2s.filter(d => d.row == r2 & d.col == c2)
	v2 = tab2.filter( d => d.row == r2 & d.col == c2)[0]
	b1s.transition().attr('fill-opacity', 0).attr('fill', 'white')
	b2s.transition().attr('fill-opacity', 0).attr('fill', 'white')
	k2.transition().attr('fill-opacity', 0.5).attr('fill', 'yellow')
	if (tab1.filter(d=>d.col==0).map(({value})=>value).indexOf(v2.value) < 0) {
		k2.transition().delay(500).attr('fill', 'red').attr('fill-opacity', 0.5)
		setTimeout(() => move(0, r2, true, false), 750)
	} else {
		k2.transition().delay(500).attr('fill', 'green').attr('fill-opacity', 0.5)
	}
}
...
function move(row1, row2, r1null=false, r2null=false) {
	...
	lsclone.transition().duration(1000).attr('transform', `translate(${width/2}, 0)`).attr('y', res_start_y + res_row * height + height / 2 - 2)
	...
	rsclone.transition().duration(1000).attr('transform', `translate(-${width/2}, 0)`).attr('y', res_start_y + res_row * height + height / 2 - 2)
...
function generate_transitions() {
	...
	for ( let r1=1; r1 < 5; r1++) {
		trans.push({transitionForward: () => rlight(r1,0), index: index_val});
		index_val++;
	}
	return trans
