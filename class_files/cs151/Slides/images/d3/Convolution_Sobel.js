...
for (var i = 0; i < 5; i++){
	...
	data[3+i*5].value = 200;
	data[4+i*5].value = 200;
}
	...
	for (var r = row; r < row + size; r++){
		...
		for (var c = col; c < col + size; c++){
			...
			if ((r >= 0 && r < dataWidth) && (c>=0 && c < dataWidth)){
				...
				var kindex = kr*size + kc
				//console.log(r, c, index, kindex, data[index].value, kernel[kindex].value);
				total += data[index].value * kernel[kindex].value;
			}
		}
	}
	return total
